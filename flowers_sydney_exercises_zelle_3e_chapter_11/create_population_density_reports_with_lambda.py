""" Print population density reports with lambda instead of Python functions. """
...
def main():
    ...
    countries.sort(key=lambda country: country.country_name)
    print_report(countries, 'BY COUNTRY NAME')

    countries.sort(key=lambda country: country.calculate_population_density_per_square_mile(), reverse=True)
