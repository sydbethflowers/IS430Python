...
def assert_equal_int(expected, actual, acceptable_error):
    """
    Compares expected and actual values; prints result (passed or failed) on the console.

    Use this function to compare int value objects.
    :param expected:  the expected int value
    :param actual:    the actual int value
    :param acceptable_error: an int value that represents the allowable difference
                             that is acceptable between expected and actual values
    :returns:   None
    :rtype:     None
    :raises TypeError: if any of expected, actual, or acceptable_error are not int types.
    """
    if not isinstance(expected, int):
        raise TypeError(f'Expected value is not a int. It is a {type(expected)}.')
    if not isinstance(actual, int):
        raise TypeError(f'Actual value is not a int. It is a {type(actual)}.')
    if not isinstance(acceptable_error, int):
        raise TypeError(f'Acceptable error value is not a int. It is a {type(acceptable_error)}.')
    actual_error = expected - actual
    abs_actual_error = abs(actual_error)
    abs_acceptable_error = abs(acceptable_error)
    if abs_actual_error <= abs_acceptable_error:
